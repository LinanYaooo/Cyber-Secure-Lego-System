...
%     w_ref = -700 * axis(joy, 5); 
    ...
    x_mB = A * x_mB + [1;0] * u_mB;
    Y(1) = 8.075 * u_mA * delta_t +  pre_y(1);
    Y(2) = 0.7 * C * x_mB + 0.1 *  pre_y(2) + 0.1 * pre_2y(2) +  0.1 * pre_3y(2);
    Y(3) = 8.075 * u_mC * delta_t +  pre_y(3);
    residual = y_mea - Y;
    
    %%  LMS estimation of steering attack; 
    error(1) = error(1) + 2 * 0.5 * (residual(1) - error(1));
    error(2) = error(2) + 2 * 0.5 * (residual(2) - error(2));
    dell_mA(i) = error(1);
    dell_mB(i) = error(2);
    ...
    if residual(1) > 10
        ...
        if pre_Attack_FLAG(1) == 0
            driving1_attacked_benchmark = y_mea(1);
            driving1_benchmark = Y(1);
        end
    else
        Attack_FLAG(1) = 0;
    end
...
%     
    ...
    if Attack_FLAG(1)
        driving1_change = y_mea(1) - driving1_attacked_benchmark;
        y_mea(1) = driving1_benchmark + driving1_change;
...
%     
    ...
    [u_mB, error_Pre_mB, error_sum_mB] = NXT_Demo_PID(1.2, 0.03 , 0.4, theta, y_mea(2), error_Pre_mB, error_sum_mB);
    ...
    legend('estimate','measurement','real','');
    subplot(2,1,2);
    plot(t,dell_mB);
    grid;
    xlabel('Time(sec)');
