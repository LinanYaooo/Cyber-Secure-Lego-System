...
Pk = [1^2 0; 0 0];                        % Steering Motor Process noise Covariance Matrix;
...
%     end   
    ...
    Y(2) =  0.3 * C_s * x_mB +  0.3 * pre_y(2) + 0.2 * pre_2y(2) +  0.2 * pre_3y(2);
    Y(3) = 8.725 * delta_t * u_mC + pre_y(3);
    %  Get the innovation signal;
    residual = y_mea - Y;
    
    %  LMS estimation of innovation attack; 
    error(1) = error(1) + 2 * 0.3 * (residual(1) - error(1));
    error(2) = error(2) + 2 * 0.3 * (residual(2) - error(2));
    error(3) = error(3) + 2 * 0.3 * (residual(3) - error(3));
...
%      end
    ...
    [u_mB, error_Pre_mB, error_sum_mB] = NXT_Demo_PID(1.1, 0.03 , 0.3, theta, y_mea(2), error_Pre_mB, error_sum_mB);
    ...
    close all;
    t = Time_axis;
    plot(t,Ang_mea_mB,t,steering_sub)
    figure();
    subplot(2,1,1);
    plot(t,Ang_est_mB,'*',t,Ang_mea_mB,t,Ang_real_mB,'LineWidth',3);
    grid;
    title('Estimation, Measurement and Real Steering Angle');
    xlabel('Time(sec)');
    ylabel('Angle(Degree)');
    legend('Estimation','Measurement','Real');
    subplot(2,1,2);
    plot(t,dell_mB,'LineWidth',3);
    grid on;
    ...
    subplot(2,1,1);
    plot(t,Ang_est_mA,'*',t,Ang_mea_mA,t,Ang_real_mA,'LineWidth',3);
    grid;
    ...
    figure();
    subplot(2,1,1);
    plot(t,Ang_est_mC,'*',t,Ang_mea_mC,t,Ang_real_mC,'LineWidth',3);
    grid;
    xlabel('Time(sec)');
    ylabel('Angle(Degree)');
    legend('est','mea','real');
