...
%% Preparation;
function Matrices_dic_driving = NXT_Demo_Matrix_dictionary_for_driving(start, precision, tend) 
    Km = 8.475 ; % V-to-ThetaDot motor model gain parameter (degrees/V.s) 
    Tm = 0; % V-to-ThetaDot motor model time-constant (s
    COL = tf(Km,[1 0]);  % open-loop controller = approx inverse of Km0/s*(Tm0 s + 1)
    ...
    close(h)
    Matrices_dic_driving = Matrices_dic;
    save('Matrices_dic_driving.mat', 'Matrices_dic_driving');
end